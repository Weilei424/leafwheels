resource "aws_s3_bucket" "images" {
  bucket = "${var.name_prefix}-images-${var.suffix}"
...
resource "aws_cloudfront_origin_access_identity" "images" {
  comment = "OAI for ${var.name_prefix} images bucket"
}
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Condition = {
          StringEquals = {
            "aws:PrincipalTag/Purpose" = "ECSTask"
          }
        }
      }
    ]
...
resource "aws_cloudfront_distribution" "images" {
  ...
  comment         = "CloudFront distribution for ${var.name_prefix} images"
  ...
  # Cache behavior for vehicle images
  ...
  # Cache behavior for accessory images
  ordered_cache_behavior {
    path_pattern     = "/accessories/*"
    allowed_methods  = ["GET", "HEAD", "OPTIONS"]
    cached_methods   = ["GET", "HEAD"]
    target_origin_id = "S3-${aws_s3_bucket.images.bucket}"
