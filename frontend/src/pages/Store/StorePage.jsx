...
const useStoreLogic = (products) => {
    ...
    const filteredProducts = useMemo(() => {
        ...
        if (searchTerm.trim()) {
            result = result.filter(p => {
                const name = p.category === "Vehicles" ? `${p.year} ${p.make} ${p.model}` : p.name || "";
        ...
        switch (sortOrder) {
            case "priceLowHigh":
                return [...result].sort((a, b) => (a.discountPrice || a.price || 0) - (b.discountPrice || b.price || 0));
            case "priceHighLow":
                return [...result].sort((a, b) => (b.discountPrice || b.price || 0) - (a.discountPrice || a.price || 0));
            default:
    ...
    return {
        ...
        hasActiveFilters: searchTerm || selectedCategory !== "All" || statusFilter !== "available",
    };
};

const StorePage = () => {
    const { vehicles, loading, error, getAvailableVehicles, getVehiclesByStatus } = useVehicleStore();
    const [pageItems, setPageItems] = useState([]);

    const products = useMemo(
        () => vehicles.map(v => ({ ...v, category: "Vehicles" })),
        [vehicles]
    );
            ...
            >
                <div className="text-center space-y-4">
                    ...
                    <motion.button
                        ...
                        whileTap={{ scale: 0.98 }}
                        onClick={() => statusFilter === "available" ? getAvailableVehicles() : setStatusFilter(statusFilter)}
                            ...
                            {pageItems.map(product => (
                                ...
                                >
                                    {product.category === "Vehicles" ? (
                                        ...
                                        <AccessoryCard accessory={product} />
                                    )}
                                </motion.div>
                            ))}
                        </AnimatePresence>
